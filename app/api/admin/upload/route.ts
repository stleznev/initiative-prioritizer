...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validate required columns and collect IDs
    ...
    const todayDatasets = await prisma.dataset.findMany({ where: { version: { startsWith: datePrefix } } });
    ...
    // Create new dataset
    const dataset = await prisma.dataset.create({ data: { version, isActive: true } });
    // Prepare initiatives for bulk insert
